...
window.HabboAPI = (function() {
  ...
  // Adicione estas funções ao window.HabboAPI
function listAllFurniture() {
    if (!window.HabboCore || !window.HabboCore.room) {
        window.HabboUI.log('❌ Sala não carregada');
        return [];
    }
    
    const furnis = window.HabboCore.room.getAllFurnis();
    
    if (furnis.length === 0) {
        window.HabboUI.log('❌ Nenhum móvel detectado na sala');
        return [];
    }
    
    window.HabboUI.log(`🪑 Lista de todos os móveis na sala (${furnis.length}):`);
    
    // Agrupar por sprite ID
    const spriteGroups = {};
    
    furnis.forEach((furni, index) => {
        if (!spriteGroups[furni.spriteId]) {
            spriteGroups[furni.spriteId] = [];
        }
        spriteGroups[furni.spriteId].push(furni);
        
        window.HabboUI.log(`  ${index + 1}. ID:${furni.id} | Sprite:${furni.spriteId} | Pos:(${furni.x},${furni.y}) | Estado:${furni.state} | Z:${furni.z}`);
    });
    
    window.HabboUI.log('\n📊 Resumo por Sprite ID:');
    Object.keys(spriteGroups).sort((a, b) => parseInt(a) - parseInt(b)).forEach(spriteId => {
        const group = spriteGroups[spriteId];
        const states = [...new Set(group.map(f => f.state))].sort();
        window.HabboUI.log(`   Sprite ${spriteId}: ${group.length} móveis | Estados: [${states.join(', ')}]`);
    });
    
    return furnis;
}

function getFurnitureBySprite(spriteId) {
    if (!window.HabboCore || !window.HabboCore.room) {
        window.HabboUI.log('❌ Sala não carregada');
        return [];
    }
    ...
    const furnis = window.HabboCore.room.getAllFurnis();
    const matching = furnis.filter(f => f.spriteId === spriteId);
    
    if (matching.length > 0) {
        window.HabboUI.log(`🔍 Encontrados ${matching.length} móveis com Sprite ID ${spriteId}:`);
        matching.forEach(furni => {
            window.HabboUI.log(`   ID:${furni.id} Pos:(${furni.x},${furni.y}) Estado:${furni.state}`);
        });
    } else {
        window.HabboUI.log(`❌ Nenhum móvel encontrado com Sprite ID ${spriteId}`);
    }
